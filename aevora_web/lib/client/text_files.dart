...
/// استخراج النص من ملفات Word (.docx) محلياً في المتصفح.
///
/// ملفات .docx عبارة عن أرشيف ZIP يحتوي XML، والنص مخزّن بترميز Unicode
/// منطقي، لذا لا يحتاج معالجة عربية خاصة.
Future<String> docxToText(List<int> bytes) async {
  final archive = ZipDecoder().decodeBytes(bytes);
  final docEntry = archive.findFile('word/document.xml');
  if (docEntry == null) {
    throw Exception(
        'ملف Word غير صالح أو قديم (.doc). يرجى حفظه بصيغة .docx ثم إعادة الرفع.');
  }
  final raw = utf8.decode(docEntry.content as List<int>, allowMalformed: true);
  ...
  if (result.isEmpty) {
    throw Exception('لم يُعثر على نص في هذا الملف.');
  }
  return result;
}

/// فك ترميز ملف نصي (.txt) بذكاء:
/// - يتعرف على BOM لترميزات UTF-8 / UTF-16LE / UTF-16BE (ملفات ويندوز).
/// - إن لم يجد BOM فحاول UTF-8 ثم Windows-1256 (العربية على ويندوز).
String decodeTextFile(List<int> bytes) {
  ...
  if (_looksLikeCp1256(bytes)) return _decodeCp1256(bytes);
  return asUtf8;
}

String _decodeUtf16(List<int> bytes, {required Endian endian}) {
  final units = ByteData.sublistView(Uint8List.fromList(bytes));
  final buffer = StringBuffer();
  for (var i = 0; i + 1 < bytes.length; i += 2) {
    buffer.writeCharCode(units.getUint16(i, endian));
  }
  return buffer.toString();
}

bool _looksLikeCp1256(List<int> bytes) {
  var arabicHits = 0;
  for (final b in bytes) {
    if (b >= 0xC0 && b <= 0xFF) arabicHits++;
  }
  return arabicHits > 2;
}

String _decodeCp1256(List<int> bytes) {
  final buffer = StringBuffer();
  for (final b in bytes) {
    buffer.writeCharCode(_cp1256[b] ?? b);
  }
  return buffer.toString();
}

/// جدول Windows-1256 (نافذة العربية على ويندوز) — مولّد من Python.
